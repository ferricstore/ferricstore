    ...
    quote do
      ...
      describe "server command parser part 1" do
        ...
        test "parses GETEX options into typed Rust AST" do
          assert {:ok,
                  [
                    ...
                    {:command, "GETEX", ["k", "PXAT", "9999"], {:getex, "k", {:pxat, 9999}},
                     ["k"]}
                  ], ""} =
                   Parser.parse_commands(
                     "getex k\r\n" <>
          ...
          assert {:ok,
                  [
                    {:command, "GETEX", ["k", "EX", "0"],
                     {:getex, "k", {:error, "ERR invalid expire time in 'getex' command"}},
                     ["k"]},
                    {:command, "GETEX", ["k", "EX", "abc"],
                     {:getex, "k", {:error, "ERR value is not an integer or out of range"}},
                     ["k"]},
        ...
        test "parses blocking command arguments into typed Rust AST" do
          assert {:ok,
                  [
                    {:command, "BLPOP", ["a", "b", "1.5"], {:blpop, ["a", "b"], 1500},
                     ["a", "b"]},
        ...
        test "keeps blocking command parse errors inside AST" do
          assert {:ok,
                  [
                    {:command, "BLPOP", ["a", "-1"],
                     {:blpop, {:error, "ERR timeout is negative"}}, ["a"]},
          ...
          assert {:ok,
                  [
                    {:command, "HELLO", ["2"],
                     {:hello,
                      {:error,
                       "NOPROTO this server does not support the requested protocol version"}},
                     []},
                    {:command, "AUTH", [],
        ...
        test "extracts pubsub channels and patterns as ACL keys" do
          assert {:ok,
                  [
                    ...
                    {:command, "PSUBSCRIBE", ["tenant:*"], {:psubscribe, ["tenant:*"]},
                     ["tenant:*"]},
                    {:command, "PUBSUB", ["numsub", "tenant:a"],
                     {:pubsub, ["NUMSUB", "tenant:a"]}, ["tenant:a"]},
        ...
        test "parses native numeric command arguments in Rust AST" do
          assert {:ok,
                  [
                    {:command, "CAS", ["k", "old", "new", "EX", "5"],
                     {:cas, "k", "old", "new", 5000}, ["k"]},
                    {:command, "LOCK", ["k", "owner", "100"], {:lock, "k", "owner", 100}, ["k"]},
                    {:command, "EXTEND", ["k", "owner", "250"], {:extend, "k", "owner", 250},
                     ["k"]},
        ...
        test "parses hash numeric and field-count arguments into typed Rust AST" do
          assert {:ok,
                  [
                    ...
                    {:command, "HINCRBYFLOAT", ["h", "f", "1.25"],
                     {:hincrbyfloat, "h", "f", 1.25}, ["h"]},
                    ...
                    {:command, "HGETEX", ["h", "EX", "10", "FIELDS", "1", "f1"],
                     {:hgetex, "h", {:ex, 10}, ["f1"]}, ["h"]},
                    {:command, "HSETEX", ["h", "60", "f", "v"], {:hsetex, "h", 60, ["f", "v"]},
                     ["h"]}
                  ], ""} =
                   Parser.parse_commands(
                     "hincrby h f 5\r\n" <>
          ...
          assert {:ok,
                  [
                    {:command, "HINCRBY", ["h", "f", "bad"],
                     {:hincrby, "h", "f",
                      {:error, "ERR value is not an integer or out of range"}}, ["h"]},
          ...
          assert {:ok,
                  [
                    ...
                    {:command, "SPOP", ["s", "-1"],
                     ...
                     ["s"]},
                    {:command, "SSCAN", ["s", "nope"],
                     {:sscan, "s", {:error, "ERR invalid cursor"}}, ["s"]},
          ...
          assert {:ok,
                  [
                    ...
                    {:command, "ZRANGE", ["z", "0", "bad"],
                     {:zrange, "z", {:error, "ERR value is not an integer or out of range"},
                      "bad"}, ["z"]},
                    {:command, "ZPOPMAX", ["z", "-1"],
                     {:zpopmax, "z", {:error, "ERR value is not an integer or out of range"}},
                     ["z"]},
                    {:command, "ZRANDMEMBER", ["z", "2", "BAD"],
                     {:zrandmember, "z", 2, {:error, "ERR syntax error"}}, ["z"]},
                    {:command, "ZSCAN", ["z", "-1"],
                     {:zscan, "z", {:error, "ERR invalid cursor"}}, ["z"]},
          ...
          assert {:ok,
                  [
                    {:command, "SETBIT", ["bits", "bad", "1"],
                     {:setbit, "bits",
                      {:error, "ERR bit offset is not an integer or out of range"}, "1"},
                     ["bits"]},
        ...
        test "parses generic key command options into typed Rust AST" do
          assert {:ok,
                  [
                    ...
                    {:command, "COPY", ["a", "b", "REPLACE"], {:copy, "a", "b", true},
                     ["a", "b"]},
