defmodule FerricstoreServer.Integration.EdgeCasesTest.Sections.ValueSizeBoundaries do
  @moduledoc false
    ...
    quote do
      ...
      describe "TTL edge cases" do
        ...
        test "key expiring in 1ms: readable immediately, nil after expiry" do
          ...
          Ferricstore.Test.ShardHelpers.eventually(
            ...
            "key with 1ms TTL should expire",
            20,
            5
          )
        end
        ...
        test "key expiring soon is readable before expiry, nil after" do
          ...
          Ferricstore.Test.ShardHelpers.eventually(
            ...
            "key with short TTL should expire",
            1_000,
            10
          )
        end
        ...
        test "PUT then overwrite with no-expiry removes the TTL" do
          ...
          Ferricstore.Test.ShardHelpers.eventually(
            ...
            "overwrite with no-expiry should clear TTL",
            20,
            10
          )
        end
      ...
      describe "concurrent write stress" do
        ...
        test "concurrent writes and reads do not return corrupted data" do
          ...
          for i <- 1..n,
              do:
                Router.put(FerricStore.Instance.get(:default), "#{base_key}_#{i}", "seed_#{i}", 0)
          ...
          read_tasks =
            Enum.map(1..n, fn i ->
              Task.async(fn ->
                Router.get(FerricStore.Instance.get(:default), "#{base_key}_#{i}")
              end)
            end)
      ...
      describe "protocol-level size guards" do
        ...
        test "SET with key over 65,535 bytes returns ERR response", %{sock: sock} do
          ...
          assert match?({:error, _}, resp),
                 "Expected error for oversized key, got: #{inspect(resp)}"
        end
        ...
        test "SET with oversized value disconnects (TOOBIG per spec)", %{sock: sock} do
          ...
          :gen_tcp.send(
            sock,
            IO.iodata_to_binary(
              FerricstoreServer.Resp.Encoder.encode(["SET", "guard_key", oversized_value])
            )
          )
        ...
        test "MSET with oversized value disconnects (TOOBIG per spec)", %{sock: sock} do
          ...
          :gen_tcp.send(
            sock,
            IO.iodata_to_binary(
              FerricstoreServer.Resp.Encoder.encode(["MSET", "guard_key2", oversized_value])
            )
          )
