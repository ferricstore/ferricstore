defmodule FerricstoreServer.Bugs.PipelineAsyncBatchErrorTest do
  ...
  test "FLOW write pipeline batches signal lease extension and owned value writes" do
    ctx = FerricStore.Instance.get(:default)
    id = "pipe_flow_hot_write_#{System.unique_integer([:positive])}"
    partition_key = "tenant-pipe-flow-hot-write"
    key = Ferricstore.Flow.Keys.state_key(id, partition_key)
    shard = Router.shard_for(ctx, key)
    handler_id = {:pipeline_flow_hot_write_batch, self(), make_ref()}

    assert :ok =
             FerricStore.flow_create(id,
               type: "pipeline-flow-hot-write",
               state: "queued",
               partition_key: partition_key,
               now_ms: 1_000,
               run_at_ms: 1_000
    ...
    assert {:ok, [claimed]} =
             FerricStore.flow_claim_due("pipeline-flow-hot-write",
               partition_key: partition_key,
    ...
    state = connection_state(ctx)
    send_response_fn = capture_response_fn()
    handle_command_fn = flunking_handle_fn("FLOW hot write pipeline fast path")
    ...
    commands = [
      {:command, "FLOW.SIGNAL", [],
       {:flow_signal, id,
        [
          partition_key: partition_key,
          signal: "external-note",
          now_ms: 1_100
        ]}, []},
      {:command, "FLOW.EXTEND_LEASE", [],
       {:flow_extend_lease, id, claimed.lease_token,
        [
          partition_key: partition_key,
    ...
    try do
      ...
      assert {:ok, %{lease_deadline_ms: 6_200} = flow} =
               FerricStore.flow_get(id, partition_key: partition_key)

      profile_ref = get_in(flow.value_refs, ["profile", :ref])
      assert is_binary(profile_ref)
      assert {:ok, ["profile-bytes"]} = FerricStore.flow_value_mget([profile_ref])

      assert {:ok, history} = FerricStore.flow_history(id, partition_key: partition_key)
      assert Enum.any?(history, fn {_event_id, fields} -> fields["signal"] == "external-note" end)

      assert Enum.any?(drain_slot_flushes(), fn {measurements, _metadata} ->
               measurements.batch_size >= 3
             end)
    after
      :telemetry.detach(handler_id)
    end
  ...
  test "mixed pipeline batches terminal value and failure FLOW read segments" do
    ctx = FerricStore.Instance.get(:default)
    partition_key = "tenant-pipe-flow-read-terminal"
    type = "pipeline-flow-read-terminal-#{System.unique_integer([:positive])}"
    failed_id = "pipe_flow_failed_#{System.unique_integer([:positive])}"
    completed_id = "pipe_flow_completed_#{System.unique_integer([:positive])}"
    handler_id = {:pipeline_flow_read_terminal_batch, self(), make_ref()}

    assert :ok =
             FerricStore.flow_create(failed_id,
               type: type,
               state: "queued",
               partition_key: partition_key,
               now_ms: 1,
    ...
    assert :ok =
             FerricStore.flow_create(completed_id,
               type: type,
               state: "queued",
               partition_key: partition_key,
               now_ms: 2,
    ...
    assert {:ok, [failed_claim, completed_claim]} =
             FerricStore.flow_claim_due(type,
               partition_key: partition_key,
               worker: "pipeline-terminal-reader",
               limit: 2,
               now_ms: 3
             )

    assert :ok =
             FerricStore.flow_fail(failed_id, failed_claim.lease_token,
               partition_key: partition_key,
    ...
    assert :ok =
             FerricStore.flow_complete(completed_id, completed_claim.lease_token,
               partition_key: partition_key,
               fencing_token: completed_claim.fencing_token,
               now_ms: 5
             )

    assert {:ok, %{ref: value_ref}} =
             FerricStore.flow_value_put("terminal-read-value", partition_key: partition_key)
    ...
    state = connection_state(ctx)
    send_response_fn = capture_response_fn()
    handle_command_fn = flunking_handle_fn("terminal/value FLOW read segment pipeline fast path")

    commands = [
      {:command, "PING", [], :ping, []},
      {:command, "FLOW.VALUE.MGET", [], {:flow_value_mget, [value_ref], [max_bytes: 1024]},
       [value_ref]},
      {:command, "FLOW.TERMINALS", [],
       {:flow_terminals, type, [partition_key: partition_key, state: "completed", count: 10]},
       [type]},
      {:command, "FLOW.FAILURES", [],
       {:flow_failures, type, [partition_key: partition_key, count: 10]}, [type]},
      {:command, "PING", [], :ping, []}
    ]
    ...
    try do
      ...
      refute response =~ "-ERR"
      assert response =~ "+PONG\r\n"
      assert response =~ "terminal-read-value"
      assert response =~ completed_id
      assert response =~ failed_id

      assert_receive {:flow_read_batch,
                      %{
                        count: 3,
                        value_mgets: 1,
                        terminals: 1,
  ...
  test "FLOW history read pipeline falls back when hot index is partially stale" do
    ctx = FerricStore.Instance.get(:default)
    type = "pipeline-history-fallback-#{System.unique_integer([:positive])}"
    partition_key = "tenant-pipe-flow-history-fallback"
    id = "pipe_flow_history_#{System.unique_integer([:positive])}"
    ...
    assert {:ok, [claimed]} =
             FerricStore.flow_claim_due(type,
               partition_key: partition_key,
               worker: "pipeline-history-worker",
               limit: 1,
               now_ms: 2
             )
    ...
    shard = Router.shard_for(ctx, history_key)
    {flow_index, flow_lookup} = Ferricstore.Flow.OrderedIndex.table_names(ctx.name, shard)
    ...
    state = connection_state(ctx)
    send_response_fn = capture_response_fn()
    handle_command_fn = flunking_handle_fn("FLOW history read pipeline fallback")
  ...
  test "FLOW pipeline fast paths reject sandbox stores" do
    ctx = FerricStore.Instance.get(:default)
    sandbox = "sandbox_flow:" <> Integer.to_string(System.unique_integer([:positive])) <> ":"
    id = "pipe_flow_sandbox_#{System.unique_integer([:positive])}"
    partition_key = "tenant-pipe-flow-sandbox"

    state = connection_state(ctx) |> Map.put(:sandbox_namespace, sandbox)
    send_response_fn = capture_response_fn()
    handle_command_fn = flunking_handle_fn("sandbox FLOW pipeline fast path")
    ...
    assert response ==
             "-ERR FLOW commands are not supported in sandbox mode\r\n" <>
               "-ERR FLOW commands are not supported in sandbox mode\r\n"
