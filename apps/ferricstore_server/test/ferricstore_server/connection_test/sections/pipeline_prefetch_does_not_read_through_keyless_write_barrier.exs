defmodule FerricstoreServer.ConnectionTest.Sections.PipelinePrefetchDoesNotReadThroughKeylessWriteBarrier do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      test "pipelined FLOW.CLAIM_DUE BLOCK 0 waits forever and holds later commands", %{
        port: port
      } do
        sock = connect(port)
        send_raw(sock, hello3())
      ...
      test "FLOW.CLAIM_DUE BLOCK reschedules delayed job after empty wake", %{port: port} do
        ...
        type =
          "block-delayed-empty-wake:" <> Integer.to_string(System.unique_integer([:positive]))
      ...
      test "FLOW.CLAIM_DUE BLOCK schedules existing delayed jobs for any partition", %{port: port} do
        handler_id =
          {__MODULE__, self(), :flow_claim_due_block_delayed_any,
           System.unique_integer([:positive])}
        ...
        type =
          "block-delayed-any-partition:" <> Integer.to_string(System.unique_integer([:positive]))
