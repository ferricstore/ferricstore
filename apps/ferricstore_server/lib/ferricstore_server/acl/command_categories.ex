defmodule FerricstoreServer.Acl.CommandCategories do
  ...
  @flow_read ~w(
    ...
    FLOW.INFO FLOW.STUCK FLOW.HISTORY
  )
  @flow_write ~w(
    FLOW.CREATE FLOW.CREATE_MANY FLOW.SPAWN_CHILDREN FLOW.POLICY.SET FLOW.CLAIM_DUE
  ...
  @admin_commands ~w(
    ...
    CLUSTER.JOIN CLUSTER.ENABLE CLUSTER.DURABILITY CLUSTER.LEAVE CLUSTER.FAILOVER
    CLUSTER.PROMOTE CLUSTER.DEMOTE CLUSTER.ROLE
    FERRICSTORE.CONFIG FERRICSTORE.HOTNESS FERRICSTORE.METRICS FLOW.RETENTION_CLEANUP
  )

  @dangerous_commands ~w(
    FLUSHDB FLUSHALL DEBUG CONFIG KEYS SHUTDOWN SORT MIGRATE RESTORE DUMP
    CLUSTER.JOIN CLUSTER.ENABLE CLUSTER.LEAVE CLUSTER.FAILOVER CLUSTER.PROMOTE
    CLUSTER.DEMOTE FERRICSTORE.CONFIG FLOW.RETENTION_CLEANUP
  )
