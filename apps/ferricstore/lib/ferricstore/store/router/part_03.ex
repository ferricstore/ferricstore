defmodule Ferricstore.Store.Router.Part03 do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      defp read_cold_batch_file_ref_async([], _now, _min_file_ref_size, _validate_blob_ref?),
        do: []
      ...
      defp read_waraft_segment_found_values(ctx, idx, indexed_entries, value_by_key, acc) do
        ...
        materialized =
          found
          |> Enum.map(fn {_index, _ctx, _idx, _keydir, _key, _file_id, _offset, value} ->
            value
          end)
          |> then(fn values ->
      ...
      defp dedupe_cold_batch_entries(entries) do
        {unique_entries, _index_by_location, value_indexes} =
          Enum.reduce(entries, {[], %{}, []}, fn entry,
                                                 {unique_acc, index_acc, value_index_acc} ->
            ...
            case Map.fetch(index_acc, location) do
              ...
              :error ->
                ...
                {[entry | unique_acc], Map.put(index_acc, location, index),
                 [index | value_index_acc]}
            end
          end)
      ...
      defp cold_batch_entry_location(
             {_ctx, _idx, _keydir, key, path, _file_id, offset, _value_size}
           ) do
        {path, offset, key}
      end
      ...
      defp waraft_segment_blob_file_ref(ctx, idx, value, validate_blob_ref?)
