defmodule Ferricstore.Store.BlobRef do
  ...
  Refs point at a payload range in a shard append segment and carry a checksum
  so materialized reads and apply-time validation can prove byte integrity.
  """

  @enforce_keys [:checksum, :size]
  defstruct checksum: nil, size: 0, segment_id: nil, offset: nil
  ...
  def encoded_size?(size), do: size == @segment_encoded_size
  ...
  def encode!(%__MODULE__{
    ...
    <<0, ?F, ?S, ?B, ?L, ?O, ?B, 1, size::unsigned-big-64, segment_id::unsigned-big-64,
      offset::unsigned-big-64, checksum::binary>>
  end

  def encode!(_ref) do
    raise ArgumentError,
          "invalid blob ref; expected non-negative u64 fields and a 32-byte checksum"
  end
  ...
  def relative_path(%__MODULE__{segment_id: segment_id})
