defmodule Ferricstore.Flow.LMDBReplaySafeIndex do
  ...
  def read_result(_shard_data_path), do: {:error, :invalid_lmdb_replay_safe_index_path}
  ...
  def reset(shard_data_path, index)
      when is_binary(shard_data_path) and is_integer(index) and index >= 0 and
             index <= @max_index do
    marker_path = path(shard_data_path)
    with_marker_lock(marker_path, fn -> write_locked(shard_data_path, marker_path, index) end)
  end
  ...
  defp with_marker_lock(marker_path, fun) when is_function(fun, 0) do
    ...
    case :global.trans(lock, fun, [node()]) do
      :aborted -> {:error, :lmdb_replay_safe_index_lock_busy}
      result -> result
    end
  catch
    :exit, reason -> {:error, {:lmdb_replay_safe_index_lock_failed, reason}}
  end
        ...
        case Integer.parse(digits) do
          {index, ""} when index >= 0 and index <= @max_index ->
            if Integer.to_string(index) == digits,
              do: {:ok, index},
              else: {:error, :invalid_lmdb_replay_safe_index}

          _invalid ->
            {:error, :invalid_lmdb_replay_safe_index}
        end
  ...
  defp recoverable_marker_error?(:invalid_lmdb_replay_safe_index), do: true
