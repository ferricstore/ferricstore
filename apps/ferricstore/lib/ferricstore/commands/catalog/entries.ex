defmodule Ferricstore.Commands.Catalog.Entries do
  ...
  # Built-ins accepted by the native parser but not yet enriched with exact
  # Redis COMMAND metadata. Keeping this inventory here makes Catalog the one
  # source of truth for command acceptance; conservative entries never claim
  # key positions or read/write flags they cannot prove.
  @builtin_command_names ~w(
  ...
  @base_names MapSet.new(@base_commands, &String.upcase(&1.name))

  @conservative_commands (for upper <- @builtin_command_names,
                              not MapSet.member?(@base_names, upper) do
                            %{
                              ...
                              last_key: 0,
                              step: 0,
                              summary: "FerricStore built-in command #{upper}.",
                              metadata: :conservative
                            }
                          end)

  @commands @base_commands ++ @conservative_commands
