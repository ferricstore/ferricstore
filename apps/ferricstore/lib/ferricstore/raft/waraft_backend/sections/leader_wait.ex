defmodule Ferricstore.Raft.WARaftBackend.Sections.LeaderWait do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      defp max_integer(left, right) when is_integer(left) and is_integer(right),
        do: max(left, right)
      ...
      defp erase_cached_voter_nodes(shard_count)
      ...
      defp stop_namespace_batchers(shard_count)
           when is_integer(shard_count) and shard_count > 0 do
      ...
      defp positive_integer_option!(_source, value) when is_integer(value) and value > 0,
        do: value
      ...
      defp erase_waraft_option_cache(shard_count)
           when is_integer(shard_count) and shard_count > 0 do
      ...
      defp estimated_term_bytes(tuple) when is_tuple(tuple) do
        tuple
        |> Tuple.to_list()
        |> Enum.reduce(8 + tuple_size(tuple) * 4, fn item, acc ->
          acc + estimated_term_bytes(item)
        end)
      end
