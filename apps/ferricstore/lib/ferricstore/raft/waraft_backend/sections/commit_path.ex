defmodule Ferricstore.Raft.WARaftBackend.Sections.CommitPath do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      defp normalize_commit_transport_result({:error, {:call_error, reason}}, pid)
           when is_pid(pid),
      ...
      defp keep_blob_protection_after_result?({:error, :commit_unreachable_after_submit}),
        do: true

      defp keep_blob_protection_after_result?({:error, :not_leader_after_submit}), do: true

      defp keep_blob_protection_after_result?(
             {:error, {:commit_call_failed_after_submit, _reason}}
           ),
           do: true
      ...
      defp redirect_transfer_failure(_kind, {:erpc, :noconnection}),
        do: {:error, :leader_unavailable}

      defp redirect_transfer_failure(_kind, _reason), do: {:error, :leader_unavailable}

      defp normalize_transfer_result({:error, :unreachable}),
        do: {:error, "ERR shard not available"}
      ...
      defp normalize_transfer_result({:error, :not_leader}),
        do: {:error, "ERR leader unavailable"}
      ...
      defp normalize_commit_result({:error, :unreachable}),
        do: {:error, "ERR shard not available"}
      ...
      defp normalize_commit_result({:error, :commit_stalled}),
        do: {:error, "ERR leader unavailable"}
