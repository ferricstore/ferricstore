defmodule Ferricstore.Raft.StateMachine.Sections.FlowTerminal do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      defp flow_cross_terminal_prepare(
             state,
             :complete,
             %{id: id, lease_token: lease_token} = attrs
           )
      ...
      defp flow_cross_terminal_apply_plan(
        ...
        with :ok <-
               flow_apply_complete_local(child_state, record, next, partition_key, now_ms, attrs) do
          flow_apply_child_terminal_chain(state, next, "completed", now_ms)
        end
      end
      ...
      defp flow_cross_terminal_apply_plan(
        ...
        with :ok <-
               flow_apply_cancel_local(child_state, record, next, attrs, partition_key, now_ms) do
      ...
      defp flow_apply_child_terminal_chain(state, child, status, now_ms) do
        ...
        if is_binary(parent_id) and parent_id != "" and
      ...
      defp flow_maybe_apply_child_terminal(
        ...
        if parent_partition_key != partition_key and not cross_shard_pending_active?() do
          ...
          parent_state =
            if cross_shard_pending_active?() do
              cross_shard_state_for_key(
                state,
                FlowKeys.state_key(parent_id, parent_partition_key)
              )
            else
              state
            end
      ...
      defp flow_child_group_resolve(group, _status),
        do: flow_child_group_resolve_all_terminal(group)
