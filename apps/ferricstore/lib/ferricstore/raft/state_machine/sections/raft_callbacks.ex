defmodule Ferricstore.Raft.StateMachine.Sections.RaftCallbacks do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      defp normalize_generic_batch_commands(commands, %{apply_context: context}),
        do: ApplyWork.normalize_batch(context, commands)

      defp normalize_generic_batch_commands(_commands, _state),
        do: {:error, :invalid_batch_apply_budget}

      defp admit_batch_command_items(%{apply_context: context}, items),
        do: ApplyWork.admit_items(context, items)

      defp admit_batch_command_items(_state, _items),
        do: {:error, :invalid_batch_apply_budget}

      defp admit_apply_command_work(%{apply_context: context}, command),
        do: ApplyWork.admit_command(context, command)
      ...
      def apply_standalone_cross_shard(execute_fn, state) when is_function(execute_fn, 1) do
        apply_standalone(fn ->
          with_cross_shard_pending_writes(state, fn ->
            store = build_cross_shard_store(state.shard_index, state)

            try do
              ShardTransaction.with_execution_context(store, fn ->
                execute_fn.(store)
              end)
            catch
      ...
      defp with_apply_time(
             %{system_time: now_ms, expiry_wall_time: wall_time_ms} = meta,
             fun
           )
           when is_integer(now_ms) and is_integer(wall_time_ms) do
        clear_stale_pending_state()

        CommandTime.with_expiry_context(now_ms, wall_time_ms, fn ->
