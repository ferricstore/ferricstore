defmodule Ferricstore.Flow.LMDBTest.Sections.MirrorFlowReadsRejectStaleLmdbRecordFallBackBitcaskTruth do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      test "mirror flow reads reject stale LMDB record and fall back to Bitcask truth" do
        ...
        assert {:ok, []} =
                 Ferricstore.Store.Router.zset_rank_range(
                   ctx,
                   correlation_index_key,
                   0,
                   10,
      ...
      test "non-idempotent create rejects existing terminal flow while hot truth remains" do
        ...
        assert {:ok, nil} =
                 Ferricstore.Flow.get(ctx, "flow-cold-duplicate-new",
                   partition_key: partition_key
                 )
      end
      ...
      test "lineage queries post-filter stale LMDB secondary index entries" do
        ...
        assert {:ok, []} =
                 Ferricstore.Flow.by_root(ctx, "wrong-root", partition_key: partition_key)
