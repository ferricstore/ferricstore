defmodule Ferricstore.Flow.LMDBTest.Sections.PartialRetentionCleanupKeepsValuesStillReferencedTerminalState do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      test "unrelated unprojected rows do not block owned value cleanup" do
        ...
        ctx =
          Ferricstore.Test.IsolatedInstance.checkout(shard_count: 2, hot_cache_max_value_size: 1)
        ...
        :ets.insert(
          ...
          {unprojected_state_key, encoded_unprojected, 0, 0, :hot, 0,
           byte_size(encoded_unprojected)}
        )
      ...
      test "retention cleanup caps owned value cleanup per command" do
        ...
        old_value_scan_limit =
          Application.get_env(:ferricstore, :flow_lmdb_value_cleanup_scan_limit)
      ...
      test "history cold projection survives restart while include_cold reads projected events" do
        ...
        assert Enum.all?(lmdb_entries, fn {_key, value} ->
                 match?(
                   {:ok,
                    {_event_id, _event_ms, _expire_at_ms, _compound_key, {:flow_history, 0},
                     offset, value_size}}
