defmodule Ferricstore.Store.DirectTombstoneBatchGuardTest do
  ...
  @compound_ops_path Path.expand("../../../lib/ferricstore/store/shard/compound/ops.ex", __DIR__)
  @compound_promoted_path Path.expand(
  ...
  test "direct promoted compound batch put writes values in one append batch" do
    source = File.read!(@compound_promoted_path)
    body = function_body(source, "promoted_write_batch_values")
  ...
  defp function_body(source, function) do
    [_before, rest] =
      case String.split(source, "defp #{function}", parts: 2) do
        [_before, rest] -> ["", rest]
        [_source] -> String.split(source, "def #{function}", parts: 2)
      end
