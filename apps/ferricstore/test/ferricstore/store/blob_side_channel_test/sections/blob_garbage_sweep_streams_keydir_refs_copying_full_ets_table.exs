defmodule Ferricstore.Store.BlobSideChannelTest.Sections.BlobGarbageSweepStreamsKeydirRefsCopyingFullEtsTable do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      test "large direct values are persisted as blob refs and materialized on reads", %{
        ...
        assert {:ok, {blob_path, blob_offset, blob_size}} =
                 BlobStore.file_ref(ctx.data_dir, 0, ref)
      ...
      test "large duplicate values share one append segment file", %{
        ...
        assert {:ok, second_encoded_ref, second_ref} =
                 raw_disk_blob_ref(ctx, keydir, "blob:auto:b")
        ...
        assert [blob_path] =
                 Path.wildcard(Path.join(ctx.data_dir, "blob/shard_0/segments/*.bloblog"))
