defmodule Ferricstore.Store.RouterColdEmptyTest do
  ...
  test "get_with_deferred_blob_file_ref retries to newer WARaft blob location",
    ...
    :ets.insert(keydir, {key, nil, 0, LFU.initial(), stale_file_id, 0, byte_size(encoded_ref)})
  ...
  test "WARaft segment batch read missing entries emit telemetry", %{
    ...
    key =
      "waraft_batch_missing_segment:" <> Integer.to_string(:erlang.unique_integer([:positive]))
  ...
  test "WARaft segment direct read missing entries emit telemetry", %{
    ...
    key =
      "waraft_direct_missing_segment:" <> Integer.to_string(:erlang.unique_integer([:positive]))
  ...
  test "WARaft segment direct read retries when ETS changes after a stale segment miss", %{
    ...
    key = "waraft_direct_stale_segment:" <> Integer.to_string(:erlang.unique_integer([:positive]))
  ...
  test "WARaft segment batch read retries when ETS changes after a stale segment miss", %{
    ...
    key = "waraft_batch_stale_segment:" <> Integer.to_string(:erlang.unique_integer([:positive]))
  ...
  test "WARaft segment getrange missing entries emit telemetry without counting a miss", %{
    ...
    key =
      "waraft_range_missing_segment:" <> Integer.to_string(:erlang.unique_integer([:positive]))
  ...
  test "failed get_file_ref cold lookup increments keyspace misses without cold-read success", %{
    ...
    key = "cold_file_ref_missing_stats:" <> Integer.to_string(:erlang.unique_integer([:positive]))
    :ets.insert(keydir, {key, nil, 0, LFU.initial(), 99, 0, 5})
