defmodule Ferricstore.Store.BlobStoreTest.Sections.PutAppendsBlobSegmentRecordReturnsSmallRef do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      test "put appends a blob segment record and returns a small ref", %{root: root} do
        ...
        assert [] ==
                 Path.wildcard(
                   Path.join(Ferricstore.DataDir.blob_shard_path(root, 0), "**/*.blob")
                 )
      ...
      test "single-entry put_many rejects invalid payload without opening the segment", %{
        root: root
      } do
        parent = self()
      ...
      test "get_range reads only the requested segment blob slice", %{root: root} do
        ...
        assert {:ok, returned_slice} =
                 BlobStore.get_range(root, 0, ref, 128 * 1024, byte_size(slice))
      ...
      test "get_range does not scan unrelated segment bytes for a partial slice", %{root: root} do
        ...
        assert {:ok, ^slice} =
                 BlobStore.get_range(root, 0, ref, byte_size(prefix), byte_size(slice))
      ...
      test "recover_shard truncates a partial segment tail and preserves prior blobs", %{
        root: root
      } do
