defmodule Ferricstore.Store.ShardAsyncIoTest.Sections.ConcurrentWrites do
  @moduledoc false
    ...
    quote do
      ...
      describe "v2_pread_batch_async NIF" do
        ...
        test "grouped async batch pread preserves original order across paths" do
          dir =
            Path.join(System.tmp_dir!(), "async_pread_batch_grouped_#{:rand.uniform(9_999_999)}")
      ...
      describe "key-validated shard cold reads" do
        test "shard GET and local transaction reads reject mismatched cold offsets" do
          ...
          try do
            ...
            key =
              "shard_cold_stale_offset:" <> Integer.to_string(:erlang.unique_integer([:positive]))
      ...
      describe "hint recovery" do
        test "supervisor shutdown runs shard terminate and writes active hint" do
          dir =
            Path.join(System.tmp_dir!(), "shard_supervised_shutdown_#{:rand.uniform(9_999_999)}")
        ...
        test "shutdown telemetry reports warning when active file fsync fails" do
          dir =
            Path.join(System.tmp_dir!(), "shard_shutdown_fsync_fail_#{:rand.uniform(9_999_999)}")
        ...
        test "recovers shard logs in numeric file id order past five digits" do
          ...
          {:ok, {_old_offset, _old_size}} =
            NIF.v2_append_record(old_log, "rollover_key", "old", 0)
