defmodule Ferricstore.Raft.WARaftSpikeTest.Sections.OneShardWaraftAdapterBootstrapSetGet do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      test "multi-partition mode can use the custom segment log", %{root: root} do
        ...
        assert Enum.all?(segment_dirs, fn dir ->
                 [_ | _] = Path.wildcard(Path.join(dir, "*.seg"))
               end)
      end
      ...
      test "custom durable segment log rejects gaps in recovered indexes as corruption", %{
        ...
        try do
          ...
          assert_receive {:segment_log_corrupt, [:ferricstore, :waraft, :segment_log_corrupt],
                          %{count: 1},
                          %{path: ^path, reason: {:non_contiguous_record_index, 1, 3}}},
                         1_000
        after
          :telemetry.detach(handler)
        end
      ...
      test "custom durable segment log recovers double-digit segment files in numeric order", %{
        ...
        log =
          {:raft_log, log_name, :ferricstore, table, partition,
      ...
      test "public log trim reports provider failure instead of advancing the view", %{root: root} do
        ...
        File.mkdir_p!(
          Path.join(List.to_string(root), "ferricstore_waraft_spike.1/segment_projection_log")
        )
      ...
      test "custom durable segment log fails closed when split append rollback fails", %{
        ...
        previous_rollback_hook =
          Application.get_env(:ferricstore, :waraft_segment_log_rollback_hook)
        ...
        try do
          ...
          Application.put_env(
            :ferricstore,
            :waraft_segment_log_rollback_hook,
            {:fail_once, self()}
          )
          ...
          assert {:error,
                  {:rollback_failed, {:append_hook, :after_write, 2},
                   {:rollback_hook, _segment_path}}} =
      ...
      test "custom durable segment log fsyncs parent directory when append creates a segment", %{
        ...
        try do
          ...
          entry =
            {term, {make_ref(), {:write, :ferricstore_waraft_spike, "segment:new", "value"}}}
