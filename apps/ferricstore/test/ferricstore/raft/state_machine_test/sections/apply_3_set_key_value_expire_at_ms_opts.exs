defmodule Ferricstore.Raft.StateMachineTest.Sections.Apply3SetKeyValueExpireAtMsOpts do
  @moduledoc false
    ...
    quote do
      ...
      describe "apply/3 with {:set, key, value, expire_at_ms, opts}" do
        ...
        test "SET KEEPTTL preserves WARaft segment-backed cold key TTL without reading it", %{
          ...
          :ets.insert(
            ets,
            {key, nil, expire_at_ms, Ferricstore.Store.LFU.initial(), {:waraft_segment, 42}, 123,
             3}
          )
        ...
        test "mixed batch SET blob ref is visible to later RMW commands", %{
          ...
          {_new_state, result} =
            StateMachine.apply(
              %{},
              {:batch,
               [{:set_blob_ref, key, encoded_ref, 0, set_opts(%{})}, {:append, key, "!"}]},
              state
            )
        ...
        test "mixed batch locked put blob ref is visible to later RMW commands", %{
          ...
          {_new_state, result} =
            StateMachine.apply(
              %{system_time: apply_now},
              {:batch,
               [{:locked_put_blob_ref, key, encoded_ref, 0, owner_ref}, {:append, key, "!"}]},
              locked_state
            )
