defmodule Ferricstore.Raft.WARaftBackendTest.Sections.WaraftGenericBatchesCoalesceBehindInFlightFlushDefault do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      test "WARaft generic batches coalesce behind an in-flight flush by default", %{ctx: ctx} do
        ...
        previous_during_flush =
          Application.get_env(:ferricstore, :waraft_generic_batch_during_flush)
        ...
        try do
          ...
          refute_receive {:waraft_backend_batcher_call, :__commit_batch_direct__, _ref, _worker},
                         50
      ...
      test "WARaft hot put batcher queues the next slot while previous flush commits", %{ctx: ctx} do
        ...
        try do
          ...
          refute_receive {:waraft_backend_batcher_call, :__commit_put_batch_direct__, _ref,
                          _worker},
                         50
      ...
      test "WARaft hot put batcher flush waits for queued work behind in-flight flush", %{
        ctx: ctx
      } do
        previous_window = Application.get_env(:ferricstore, :waraft_hot_batch_window_ms)
        previous_hook = Application.get_env(:ferricstore, :waraft_backend_batcher_call_hook)
      ...
      test "deterministic command errors still advance WARaft storage replay position", %{
        ctx: ctx
      } do
        assert :ok = WARaftBackend.start(ctx, log_module: :ferricstore_waraft_spike_segment_log)
