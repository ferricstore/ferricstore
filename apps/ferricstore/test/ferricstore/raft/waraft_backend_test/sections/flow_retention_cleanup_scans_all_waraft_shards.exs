defmodule Ferricstore.Raft.WARaftBackendTest.Sections.FlowRetentionCleanupScansAllWaraftShards do
  @moduledoc false
    ...
    quote do
      ...
      test "Flow cross-shard terminal many commands resolve parents through WARaft", %{root: root} do
        ...
        try do
          ...
          complete_claim =
            claim_flow_child!(ctx, complete_child, complete_child_partition, "many-c")
      ...
      test "CMS Cuckoo TopK and TDigest commands use WARaft as the selected backend", %{ctx: ctx} do
        try do
          ...
          assert [3] =
                   Ferricstore.Commands.CMS.handle_ast({:cms_query, ["router:cms", "hot"]}, ctx)

          assert :ok =
                   Ferricstore.Commands.Cuckoo.handle_ast({:cf_reserve, "router:cf", 128}, ctx)

          assert 1 = Ferricstore.Commands.Cuckoo.handle_ast({:cf_add, ["router:cf", "seen"]}, ctx)

          assert 1 =
                   Ferricstore.Commands.Cuckoo.handle_ast(
                     {:cf_exists, ["router:cf", "seen"]},
                     ctx
                   )
          ...
          assert [6] =
                   Ferricstore.Commands.TopK.handle_ast({:topk_count, ["router:topk", "a"]}, ctx)
      ...
      test "file-backed probabilistic commands survive WARaft restart", %{root: root, ctx: ctx} do
        ...
        try do
          ...
          assert 1 =
                   Ferricstore.Commands.Bloom.handle_ast({:bf_add, [bloom_key, "sensor-a"]}, ctx)
      ...
      test "stream XDEL and XTRIM mutations survive WARaft restart", %{root: root, ctx: ctx} do
        try do
          ...
          assert 2 =
                   Ferricstore.Commands.Stream.handle_ast({:xtrim, key, {:maxlen, false, 2}}, ctx)
      ...
      test "hash field metadata reads survive WARaft restart", %{root: root, ctx: ctx} do
        try do
          ...
          assert [ttl_before] =
                   Ferricstore.Commands.Hash.handle_ast({:hpttl, key, ["field"]}, ctx)
      ...
      test "advanced hash mutations survive WARaft restart", %{root: root, ctx: ctx} do
        ...
        try do
          ...
          assert ["gone", nil] =
                   Ferricstore.Commands.Hash.handle_ast(
                     {:hgetdel, key, ["delete", "missing"]},
                     ctx
                   )

          assert [1] =
                   Ferricstore.Commands.Hash.handle_ast({:hpexpire, key, 60_000, ["int"]}, ctx)
