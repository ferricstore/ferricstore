defmodule Ferricstore.Raft.WARaftBackendTest.Sections.StartupFinalizesInterruptedSnapshotSwapAfterMetadataPersisted do
  @moduledoc false
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      test "startup finalizes interrupted snapshot swap after metadata persisted", %{root: root} do
        ...
        assert nil ==
                 latest_storage_metadata_journal(
                   waraft_storage_metadata_journal_path(target_root, 0)
                 )
      ...
      test "startup fails closed when pending snapshot install metadata is unreadable", %{
        root: root
      } do
      ...
      test "startup fails closed when pending snapshot install marker position differs from metadata",
        ...
        File.write!(
          Path.join(root_dir, "ferricstore_storage.term"),
      ...
      test "snapshot install clears stale Flow apply projection cache for target root", %{
        root: root
      } do
      ...
      test "backend async submit does not assert on commit_async after reserving bytes" do
        ...
        assert [submit_source, _] =
                 String.split(submit_source, "defp await_commit_async", parts: 2)
      ...
      test "membership storage config polling wraps WARaft exits" do
        ...
        assert [_, participant_source] =
                 String.split(source, "defp storage_participant?", parts: 2)

        assert [participant_source, member_and_rest] =
                 String.split(participant_source, "defp wait_storage_member", parts: 2)

        assert [_, member_source] =
                 String.split(member_and_rest, "defp storage_member?", parts: 2)
      ...
      test "startup promotion wraps WARaft exits" do
        ...
        assert finish_source =~
                 "backend_call(fn -> :wa_raft_server.promote(server, :next, true) end)"
      end
    end
  end
end
