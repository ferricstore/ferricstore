defmodule Ferricstore.Commands.ServerInfoTest do
  ...
  describe "INFO (all sections)" do
    test "keyspace expiry stats use the same HLC clock as expiry decisions" do
      ...
      assert body =~ "HLC.now_ms()"
      refute body =~ "System.os_time(:millisecond)"
    end
  ...
  describe "INFO bitcask" do
    test "exposes release cursor frontier metrics per shard" do
      ...
      assert result =~ "shard_0_replay_safe_index:"
      assert result =~ "shard_0_replay_safe_requested_index:"
      assert result =~ "shard_0_replay_safe_lag:"
      assert result =~ "shard_0_replay_safe_persist_failures:"
      assert result =~ "shard_0_flow_lmdb_replay_safe_index:"
      assert result =~ "shard_0_flow_lmdb_replay_safe_requested_index:"
      assert result =~ "shard_0_flow_lmdb_replay_safe_lag:"
      assert result =~ "shard_0_flow_lmdb_replay_safe_persist_failures:"
      assert result =~ "shard_0_flow_lmdb_mirror_enqueue_failures:"
      assert result =~ "shard_0_flow_lmdb_mirror_degraded:"
      assert result =~ "shard_0_flow_lmdb_writer_pending_ops:"
      assert result =~ "shard_0_flow_lmdb_writer_oldest_pending_age_us:"
      assert result =~ "shard_0_flow_lmdb_writer_flush_failures:"
