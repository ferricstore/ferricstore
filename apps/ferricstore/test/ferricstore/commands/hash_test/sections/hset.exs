defmodule Ferricstore.Commands.HashTest.Sections.Hset do
  @moduledoc false
    ...
    quote do
      ...
      describe "HSET" do
        ...
        test "HSET builds write entries and added count in one pass" do
          source =
            File.read!(
              Path.expand("../../../lib/ferricstore/commands/hash/field_ops.ex", __DIR__)
            )
        ...
        test "multi-field hash metadata writes are built in one pass" do
          ...
          assert source =~
                   "persistent_field_entries(unique_fields, compound_keys, metas_by_field, [])"
        ...
        test "hash field-value responses use shared flat-list helper" do
          source =
            File.read!(Path.expand("../../../lib/ferricstore/commands/hash.ex", __DIR__)) <>
              File.read!(
                Path.expand("../../../lib/ferricstore/commands/hash/helpers.ex", __DIR__)
              )
