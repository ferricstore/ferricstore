    ...
    quote do
      ...
      describe "TDIGEST.CDF" do
        ...
        test "handles multiple CDF values in one call" do
          ...
          results =
            TDigestCmd.handle("TDIGEST.CDF", ["mydigest", "250.0", "500.0", "750.0"], store)
      ...
      describe "TDIGEST.MERGE" do
        ...
        test "merges three digests" do
          ...
          :ok =
            TDigestCmd.handle(
              "TDIGEST.ADD",
              ["s1" | Enum.map(1..333, &Integer.to_string/1)],
              store
            )
          ...
          :ok =
            TDigestCmd.handle(
              "TDIGEST.ADD",
              ["s2" | Enum.map(334..666, &Integer.to_string/1)],
              store
            )
        ...
        test "merges with different compressions" do
          ...
          :ok =
            TDigestCmd.handle(
              "TDIGEST.ADD",
              ["src1" | Enum.map(1..100, &Integer.to_string/1)],
              store
            )
        ...
        test "merge count = sum of source counts" do
          ...
          :ok =
            TDigestCmd.handle(
              "TDIGEST.ADD",
              ["src1" | Enum.map(1..30, &Integer.to_string/1)],
              store
            )
          ...
          :ok =
            TDigestCmd.handle(
              "TDIGEST.ADD",
              ["src2" | Enum.map(1..70, &Integer.to_string/1)],
              store
            )
        ...
        test "returns error for non-existent source key" do
          ...
          assert {:error, msg} =
                   TDigestCmd.handle("TDIGEST.MERGE", ["dst", "1", "missing"], store)
      ...
      describe "edge cases" do
        ...
        test "empty digest handles all query commands gracefully" do
          ...
          assert "nan" =
                   TDigestCmd.handle("TDIGEST.TRIMMED_MEAN", ["mydigest", "0.0", "1.0"], store)
