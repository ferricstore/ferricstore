defmodule Ferricstore.Commands.GenericTest.Sections.Type do
  @moduledoc false
    ...
    quote do
      ...
      describe "RENAMENX" do
        ...
        test "RENAMENX errors when source doesn't exist" do
          ...
          assert {:error, "ERR no such key"} =
                   Generic.handle("RENAMENX", ["missing", "new"], store)
        end
        ...
        test "COPY with REPLACE preserves compound destination when compound write fails" do
          ...
          store =
            Map.put(base, :compound_batch_put, fn
              "dst", entries ->
                if Enum.any?(entries, fn {_compound_key, value, _expire_at_ms} ->
                     value == "new"
                   end) do
                  {:error, :disk_full}
                else
      ...
      describe "SCAN" do
        ...
        test "SCAN with MATCH pattern filters keys" do
          ...
          [_cursor, keys] =
            Generic.handle("SCAN", ["0", "MATCH", "user:*", "COUNT", "100"], store)
        ...
        test "SCAN with COUNT hint limits batch size" do
          data =
            for i <- 1..20, into: %{}, do: {"k#{String.pad_leading("#{i}", 2, "0")}", {"v", 0}}
      ...
      describe "OBJECT FREQ" do
        ...
        test "OBJECT FREQ returns error for missing key" do
          ...
          assert {:error, "ERR no such key"} =
                   Generic.handle("OBJECT", ["FREQ", "missing"], store)
        end
      end
    end
  end
