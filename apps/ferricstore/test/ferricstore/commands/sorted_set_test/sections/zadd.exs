    ...
    quote do
      ...
      describe "ZADD" do
        ...
        test "ZADD builds batch write entries without zip flat_map" do
          ...
          assert source =~
                   "zset_write_entries(unique_members, compound_keys, writes_by_member, [])"
        ...
        test "WITHSCORES responses use shared flat-list helpers" do
          source =
            File.read!(Path.expand("../../../lib/ferricstore/commands/sorted_set.ex", __DIR__)) <>
              File.read!(
                Path.expand("../../../lib/ferricstore/commands/sorted_set/helpers.ex", __DIR__)
              )
        ...
        test "ZRANGE negative bounds use score-index cardinality when available" do
          ...
          store =
            ...
            |> Map.put(:compound_count, fn _redis_key, _prefix ->
              flunk(
                "ZRANGE should not scan/count zset members when score-index cardinality exists"
              )
            end)
      ...
      describe "ZREM" do
        ...
        test "ZREM cleanup uses score-index cardinality when available" do
          ...
          store =
            ...
            |> Map.put(:compound_count, fn _redis_key, _prefix ->
              flunk(
                "ZREM cleanup should not scan/count members when score-index cardinality exists"
              )
            end)
      ...
      describe "ZCOUNT" do
        ...
        test "ZCOUNT on nonexistent key returns 0" do
          assert 0 ==
                   SortedSet.handle("ZCOUNT", ["nonexistent", "-inf", "+inf"], MockStore.make())
        end
        ...
        test "ZRANGEBYSCORE pushes LIMIT into score-index slice when available" do
          ...
          store =
            Map.put(base_store, :zset_score_range_slice, fn redis_key,
              ...
              send(
                parent,
                {:zset_score_range_slice, redis_key, min_bound, max_bound, reverse?, offset,
                 count}
              )
          ...
          assert_receive {:zset_score_range_slice, "zs", {:inclusive, 1.0}, {:inclusive, 3.0},
                          false, 1, 1}
        end
