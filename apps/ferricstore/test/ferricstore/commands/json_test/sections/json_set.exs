defmodule Ferricstore.Commands.JsonTest.Sections.JsonSet do
  @moduledoc false
    ...
    quote do
      ...
      describe "JSON.SET" do
        ...
        test "unrecognized flag returns error" do
          assert {:error, msg} =
                   Json.handle("JSON.SET", ["k", "$", "1", "BOGUS"], MockStore.make())
      ...
      describe "JSON.ARRAPPEND" do
        ...
        test "returns error for invalid JSON value" do
          ...
          assert {:error, msg} =
                   Json.handle("JSON.ARRAPPEND", ["doc", "$.arr", "not{json"], store)
