defmodule Ferricstore.Cluster.NodeJoinSyncTest.Sections.NewNodeJoinContinuousWrites do
  @moduledoc false
    ...
    quote do
              ...
              if missing != [] and (pc == 1 or pc == 5 or rem(pc, 50) == 0) do
                ...
                for k <- sample do
                  ...
                  if ctx do
                    ...
                    IO.puts(
                      "    missing key #{inspect(k)} shard=#{shard} ets=#{inspect(ets_entry)}"
                    )
                  end
                end
              end
      ...
      describe "standalone to cluster (manual join)" do
        ...
        test "single node with data → two nodes join via CLUSTER.JOIN → all data synced" do
          ...
          eventually(
            fn ->
              assert Enum.all?(all_keys, fn k -> read_key(node_b, k) != nil end),
                     "keys missing on b"

              assert Enum.all?(all_keys, fn k -> read_key(node_c, k) != nil end),
                     "keys missing on c"
            end,
      ...
      describe "disk snapshot: new node boots from copied disk while writes continue" do
        ...
        test "node with cloned disk joins cluster and gets all data" do
          ...
          clone_dir =
            Path.join(
              System.tmp_dir!(),
              "ferricstore_clone_#{System.unique_integer([:positive])}"
            )
      ...
      describe "duplicate join is idempotent" do
        ...
        test "concurrent add_node calls for same target converge" do
          ...
          for shard_idx <- 0..(shard_count - 1) do
            {:ok, members, _leader} =
              :erpc.call(n_a, Ferricstore.Raft.Cluster, :members, [shard_idx])
