defmodule Ferricstore.Bench.EmbeddedWriteProfileTest do
  ...
  test "profile embedded SET - every layer timed" do
    ...
    {router_us, _} =
      :timer.tc(fn ->
        for i <- 1..@iterations,
            do: Router.put(FerricStore.Instance.get(:default), "#{prefix}:rtr:#{i}", value, 0)
      end)
    ...
    {hash_us, _} =
      :timer.tc(fn ->
        for _ <- 1..@iterations,
            do: Router.shard_for(FerricStore.Instance.get(:default), "somekey")
      end)
  ...
  test "throughput: single writer vs N concurrent writers (quorum)" do
    ...
    IO.puts(
      "  #{String.pad_trailing("Writers", 10)} #{String.pad_leading("Writes/sec", 12)} #{String.pad_leading("Per-writer", 12)} #{String.pad_leading("us/op", 10)}"
    )
    ...
    for num_writers <- [1, 2, 4, 8, 16, 32, 50] do
      ...
      IO.puts(
        "  #{String.pad_trailing("#{num_writers}", 10)} #{String.pad_leading("#{wps}", 12)} #{String.pad_leading("#{per_writer}", 12)} #{String.pad_leading("#{us_per_op}", 10)}"
      )
    end
  end
  ...
  test "comparison: FerricStore.set vs Router.put vs direct ETS (50 writers, 3s)" do
    ...
    IO.puts(
      "  #{String.pad_trailing("Method", 30)} #{String.pad_leading("Writes/sec", 12)} #{String.pad_leading("us/op", 10)}"
    )

    IO.puts("  #{String.duplicate("-", 55)}")

    for {label, fun_maker} <- [
          {"FerricStore.set (quorum)",
           fn prefix ->
      ...
      IO.puts(
        "  #{String.pad_trailing(label, 30)} #{String.pad_leading("#{wps}", 12)} #{String.pad_leading("#{us_per_op}", 10)}"
      )
    end
  end
end
