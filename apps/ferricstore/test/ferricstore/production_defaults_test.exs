defmodule Ferricstore.ProductionDefaultsTest do
  ...
  test "production and benchmark config default to the production WARaft path" do
    ...
    assert config_exs =~ "flow_retention_sweeper_catchup_burst_limit: 8"
    assert config_exs =~ "flow_retention_sweeper_catchup_pause_ms: 1_000"
    ...
    assert bench_exs =~ "flow_retention_sweeper_catchup_burst_limit: 8"
    assert bench_exs =~ "flow_retention_sweeper_catchup_pause_ms: 1_000"
    ...
    refute runtime_exs =~ "FERRICSTORE_FLOW_LMDB_ENABLED"
