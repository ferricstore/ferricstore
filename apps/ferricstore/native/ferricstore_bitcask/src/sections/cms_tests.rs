...
#[test]
fn transactional_incrby_replays_the_same_token_without_double_counting() {
    let dir = tempfile::tempdir().unwrap();
    let path_str = create_cms_file(dir.path(), "transactional.cms", 100, 5);
    let path = Path::new(&path_str);
    let file = crate::open_random_rw_locked(path).unwrap();
    let (width, depth, _) = cms_file_read_header(&file).unwrap();
    let items = [(b"hello".as_slice(), 3_i64)];
    let token = crate::prob_txn::MutationToken::new(41, 1);

    assert_eq!(
            cms_transactional_incrby(&file, path, width, depth, &items, token).unwrap(),
        vec![3]
    );
    assert_eq!(
        cms_transactional_incrby(&file, path, width, depth, &items, token).unwrap(),
    ...
    assert_eq!(
        cms_query_counts(&file, width, depth, [b"hello".as_slice()]).unwrap(),
        vec![3]
    );
}
...
#[test]
fn merge_total_count_rejects_a_negative_result() {
    let error = cms_next_merge_total_count(5, 10, -1).unwrap_err();
...
fn merge_uses_bounded_chunks_instead_of_one_syscall_per_counter() {
    ...
    let merge = source
        .split("fn cms_stage_merge(")
        .nth(1)
        .unwrap()
        .split("fn cms_apply_merge_plan(")
        .next()
        .unwrap();
