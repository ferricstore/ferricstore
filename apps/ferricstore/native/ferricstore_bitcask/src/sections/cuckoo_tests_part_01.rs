...
fn file_create_and_read_header() {
    ...
    let file_size = HEADER_SIZE + bucket_bytes + crate::prob_txn::TOKEN_SIZE;
...
fn file_alternate_bucket_is_involution() {
    ...
    for num_buckets in [1, 3, 10, 127, 1024] {
        for i in 0..1000 {
            ...
            let (fp, b1) = cuckoo_file_fingerprint_and_bucket(elem.as_bytes(), 1, num_buckets);
...
fn create_cuckoo_file(dir: &std::path::Path, name: &str, capacity: u32, bucket_size: u8) -> String {
