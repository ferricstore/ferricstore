...
- Added metadata-row and composite-covering execution for built-in fields plus bounded `attribute.*` and `state_meta.*` values. Eligible projections and counts perform no authoritative-log reads, while full-record queries use byte-, memory-, concurrency-, and deadline-bounded grouped reads with identity, version, expiry, size, and checksum validation.
...
- Added result-equivalence, restart, compaction-race, malformed-input, memory-pressure, soak, and storage-layout benchmarks. At 10,000 records the compact projection reduced physical LMDB bytes by about 23.0% for sparse, 35.5% for typical, and 6.0% for metadata-heavy records; covered 100-row projections measured about 5-10x faster than authoritative hydration, and spilled retention reduced peak memory by 62.9% in the 10,000-record fixture.
